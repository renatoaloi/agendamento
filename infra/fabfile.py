...
def _load_data_fixtures(src_folder):
    run(f'cd {src_folder} && ../virtualenv/bin/python manage.py loaddata especialidades profissionais')
...
def _configure_gunicorn(app_folder, src_folder, app_name):
    ...
    run(f'sudo sed -i "s/APPPATH=.*$/APPPATH={escaped_app_folder}/g" /etc/init.d/{app_name}')
    escaped_src_folder = src_folder.replace('/', '\\/')
    run(f'sudo sed -i "s/SRCPATH=.*$/SRCPATH={escaped_src_folder}/g" /etc/init.d/{app_name}')
